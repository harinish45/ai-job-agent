              ...
              <div key={app.id} className="card">
                <div className="flex items-start justify-between">
                  ...
                  <div className="relative group">
                    <button
                      className="btn-secondary text-xs px-3 py-1.5 flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:outline-none"
                      aria-haspopup="true"
                      aria-label={`Update status for ${app.job_title} at ${app.company}`}
                    >
                      Update
                      <ChevronDown size={12} />
                    </button>
                    <div className="absolute right-0 mt-1 w-40 bg-white rounded-lg shadow-lg border border-gray-200 hidden group-hover:block group-focus-within:block z-10">
                      {Object.entries(statusConfig).map(([key, cfg]) => (
                        <button
                          key={key}
                          onClick={() => updateStatus.mutate({ id: app.id, status: key })}
                          className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 focus:bg-gray-50 focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:outline-none focus:relative z-20 first:rounded-t-lg last:rounded-b-lg"
                        >
                          {cfg.label}
                        </button>
                      ))}
